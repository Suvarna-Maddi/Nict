...
export function WhyChooseUs() {
  return (
    <section className="relative w-full py-20 z-10 overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-[600px] bg-red-50/40 rounded-full blur-[150px] pointer-events-none -translate-y-1/2"></div>
      <div className="absolute bottom-0 right-0 w-[800px] h-[800px] bg-red-100/30 rounded-full blur-[150px] pointer-events-none translate-x-1/4 translate-y-1/4"></div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16 relative z-10">
          <h2 className="text-5xl md:text-7xl font-black text-slate-900 tracking-tighter mb-6">
            Why <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-600 to-red-400">Choose Us</span>
          </h2>
        ...
        <div className="relative w-full max-w-5xl mx-auto">
          ...
          <div className="absolute left-1/2 top-0 bottom-0 w-1 bg-gradient-to-b from-red-100 via-red-300 to-red-100 -translate-x-1/2 rounded-full opacity-50"></div>
            ...
            {highlights.map((item, idx) => {
              ...
              return (
                <div key={idx} className={`relative flex items-center w-full group ${isLeft ? 'flex-row' : 'flex-row-reverse'}`}>
                  ...
                  <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 md:w-8 md:h-8 bg-white border-4 border-red-500 rounded-full shadow-[0_0_20px_rgba(220,38,38,0.5)] z-20 group-hover:scale-150 group-hover:bg-red-50 transition-all duration-500"></div>
                  ...
                  <div className={`w-1/2 flex ${isLeft ? 'justify-end pr-8 md:pr-20' : 'justify-start pl-8 md:pl-20'} relative`}>
                    ...
                    <div className={`relative z-10 flex flex-col ${isLeft ? 'items-end text-right' : 'items-start text-left'} group-hover:scale-[1.02] transition-transform duration-500 ease-out`}>
                      ...
                      <div className="relative mb-2 md:mb-4 inline-block">
                        <h3 className="text-xl md:text-4xl font-extrabold text-slate-900 tracking-tight group-hover:text-red-600 transition-colors duration-300">
                          {item.title}
                        </h3>
                        {/* Animated Underline */}
                        <div className={`absolute -bottom-2 ${isLeft ? 'right-0' : 'left-0'} w-0 h-1 bg-red-500 rounded-full group-hover:w-full transition-all duration-500 ease-out`}></div>
                      </div>
